...
/**
 * Hero v5 — Visibility floor + directional drift + narrative transition
 *
 * v4: Live topology canvas, ambient network, pipeline gradient text
 * v5 changes:
 *   - Alpha floor raised: depthAlpha 0.25 min — no ghost nodes
 *   - Subtle downward drift — network flows like maturing knowledge
 *   - Connection alpha raised to match brighter nodes
 *   - Transition text rewritten: raw → agent-ready, invitation to follow
 *   - For humans, not about the system
 */
...
export default function HeroV5() {
  ...
  // Mini-topology canvas — ambient background with directional flow
  useEffect(() => {
    ...
    // Create nodes — ambient feel with subtle downward drift
    const nodeCount = 35;
    ...
    const nodes: Node[] = Array.from({ length: nodeCount }, () => {
      ...
      return {
        ...
        // v5: downward drift — knowledge flows down, deeper nodes drift faster
        vy: (Math.random() - 0.5) * (0.10 + depth * 0.06) + 0.03 + depth * 0.02,
        r: 1.2 + (1 - depth) * 2,
        depth,
        tier: pickTier(),
    ...
    const draw = () => {
      ...
      for (let i = 0; i < nodes.length; i++) {
        for (let j = i + 1; j < nodes.length; j++) {
          ...
          if (d < connectionDistance) {
            ...
            const baseAlpha = fade * (0.03 + (1 - avgDepth) * 0.06);
      ...
      nodes.forEach(n => {
        ...
        const depthAlpha = 0.25 + (1 - n.depth) * 0.20;
        ...
        if (finalAlpha > 0.12) {
          ...
          ctx.fillStyle = `rgba(${c.r},${c.g},${c.b},${finalAlpha * 0.06})`;
          ctx.fill();
        }
    ...
    <main className="bg-penguin-bg text-gray-100">
      ...
      >
        ...
        <div className="relative z-10 flex flex-col items-center text-center px-6 max-w-3xl mx-auto">
          ...
          >
            <span>
              <strong className="text-emerald-400/80 font-semibold">19K+</strong> chunks
            </span>
            ...
            <span className="w-px h-3 bg-gray-800" />
            <span>
              <strong className="text-amber-400/80 font-semibold">121</strong> skills
            </span>
            <span className="w-px h-3 bg-gray-800" />
            <span>
      ...
      >
        <div className="text-center max-w-lg" data-reveal>
          <p className="text-gray-400 text-base sm:text-lg leading-loose">
            Raw knowledge becomes agent-ready.
          </p>
          <p className="text-gray-600 text-sm mt-3 leading-relaxed">
            Follow the path — we&apos;ll show you&nbsp;how.
          </p>
        </div>
      </section>
